...
fn translate_node(hir_node_idx: NodeIndex, tr: &FunctionTranslation) -> Result<(), Error> {
    ...
    match hir_node {
        ...
        hir::Node::FuncPrim(hir::Prim::Prim(uiua::Primitive::Not)) => {
            let [input_info] = tr.infos([inputs[0]]);
            let output_info = not(input_info, ctx)?;
            let [output] = tr.add_node(
                mir::Node::FuncPrim(mir::Prim::Prim(uiua::Primitive::Not)),
                [output_info],
                inputs,
            );
...
fn not(input_info: &ValueInfo, ctx: AnalyzeContext) -> Result<ValueInfo, Error> {
